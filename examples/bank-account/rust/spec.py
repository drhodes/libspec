...
class TestSuite(SystemRequirement):
    ...
    def action(self): return "build rust tests for all endpoints"
    def benefit(self): return "ensure API behaves as expected"


class TwoFile(SystemRequirement):
    def req_id(self):  return "REQ-006"
    def title(self):   return "two-source-files"
    def actor(self):   return "LLM"
    def action(self):  return "generate API spec in a two source files"
...
class ProgrammingLanguage(SystemRequirement):
    ...
    def actor(self):   return "LLM"
    def action(self):  return "Uses the Rust programming langauge"
