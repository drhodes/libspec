...
class TicTacToeGame:
    ...
    def _create_ui(self):
        ...
        # Create 3x3 grid of buttons with equal sizing
        self.buttons = []
        grid_rows = []
        
        # Fixed width for each button to ensure square grid
        button_width = 7
        ...
        for row in range(3):
            ...
            for col in range(3):
                ...
                # Use fixed width to ensure all buttons are the same size
                btn_padded = ('fixed', button_width, btn)
                button_row.append(btn_padded)
                self.buttons.append((row, col, btn))
            # Use Columns with dividechars=1 for spacing between buttons
            grid_rows.append(urwid.Columns(button_row, dividechars=1))
        
        # Use Pile with dividechars=0 to keep rows compact (square appearance)
        grid = urwid.Pile(grid_rows)
        grid_box = urwid.LineBox(grid, title='Tic-Tac-Toe')
        
        # Control buttons with equal sizing
        button_width = 12
        ...
        # Use fixed width for all control buttons to ensure equal sizing
        controls = urwid.Columns([
            ('fixed', button_width, urwid.AttrMap(new_game_btn, 'button', 'button_focus')),
            ('fixed', button_width, urwid.AttrMap(save_btn, 'button', 'button_focus')),
            ('fixed', button_width, urwid.AttrMap(load_btn, 'button', 'button_focus')),
            ('fixed', button_width, urwid.AttrMap(quit_btn, 'button', 'button_focus')),
        ], dividechars=1)
        
        
        # Center elements by calculating left padding to center fixed-width content
        # Use Padding with 'center' align and specify the actual width of content
        grid_centered = urwid.Padding(grid_box, align='center', width='pack', left=0, right=0)
        controls_centered = urwid.Padding(controls, align='center', width='pack', left=0, right=0)
        status_centered = urwid.Padding(self.status_text, align='center', width='pack', left=0, right=0)
        
        # Main layout with all elements centered
        layout = urwid.Pile([
            urwid.Divider(),
            status_centered,
            urwid.Divider(),
        ...
        self.main_widget = urwid.Filler(layout, valign='middle')
