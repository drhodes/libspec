...
class TicTacToeUI:
    ...
    def __init__(self):
        ...
        self.board_widgets = self._make_board_widgets()          # 9 Buttons

        # ---- Build a 3×3 grid ------------------------------------
        rows = []
        for r in range(3):
            row_buttons = self.board_widgets[r * 3 : (r + 1) * 3]
            cols = urwid.Columns(row_buttons, dividechars=1, align='center')
            rows.append(cols)
        board_grid = urwid.Pile(rows)   # <-- proper grid
