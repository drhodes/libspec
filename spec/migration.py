...
class MigrationTool(Feat):
    ...
    to the new unified version 5.0.0 storage layer (SpecStore database or
    active XML directory).
    """
...
class MigrateSwitch(Req):
    ...
    The switch must:
    1. Scan the specified version 4 build directory for all historical
    `spec-*.xml` files. 2. Extract all specifications, docstrings,
    relationships, and metadata chronologically. 3. Initialize the target
    active SpecStore (either a clean SQLite/Postgres database as determined by
    LIBSPEC_DATABASE_URL, or a new version 5 directory). 4. Deterministically
    populate the store with all parsed historical snapshots in the exact
...
class MigrationVerification(Req):
    ...
    1. Configure an isolated, clean temporary SQLite SpecStore database. 2.
    Execute `libspec migrate` pointing to `tests/spec-build` containing 12
    historical XML spec snapshots. 3. Assert that all 12 snapshots are
