...
class McpAgentCommand(Feat):
    """`libspec mcp_agent (<agent> [DIR] | --list)` automates local coding
    agent integrations.
...
class MigrateCommand(Feat):
    """`libspec migrate <v4_build_dir>` upgrades workspace assets from v4.2.0
    (XML hashed files) to version 5.0.0 (unified SpecStore).
...
class ReplCommand(Feat):
    """`libspec repl` launches the interactive specification inspector REPL
    shell.
