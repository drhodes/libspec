...
class BuiltInVocabulary(Req):
    """
    Libspec ships a library of Ctx-derived base classes that projects can use
    to structure their specifications. These are the canonical spec types
...
class PeeWeeType(Feat):
    """
    PeeWee extends DataSchema with guidance to implement the schema using the
    PeeWee ORM and write tests verifying database behavior.
...
class RestMixinType(Feat):
    """
    RestMixin is a plain Ctx mixin carrying the guidance to develop a REST API
    with best practices around the interface.
...
class ImplementationType(Feat):
    """
    Implementation extends Requirement with guidance that implementations must
    include tests and specifies the target directory.
...
class LeafMethodsMixin(Feat):
    ...
    introspection for API and CmdLine spec classes.
