...
from libspec.mcp_server import (
    compact_store,
    delete_snapshot,
    link_snapshot,
    restore_snapshot,
...
def test_repl_link_command():
    ...
    with runner.isolated_filesystem():
        ...
        res = repl.commander.run(
            f"link --snapshot {snap.id} --vcs git --revision abcdef123 --metadata author=derek",
            repl,
        )
        assert res is True
...
def test_mcp_lifecycle_tools():
    ...
    with runner.isolated_filesystem():
        ...
        link_res = link_snapshot(
            snapshot_id=snap1.id,
            vcs="git",
            revision="abc12345",
            metadata={"author": "derek"},
        )
