...
def test_jsonlines_store_basic_flow(tmp_path):
    ...
    snap = store.store_snapshot(
        [comp1, comp2], git_commit="c" * 40, created_at=created_at
    )
...
def test_jsonlines_canonical_serialization(tmp_path):
    ...
    assert (
        " " not in raw_snap_line
    )  # Except within docstring / string values, but there are no spaces in keys/values of snapshot record
...
def test_jsonlines_claim_deduplication(tmp_path):
    ...
    claim1 = Implemented(
        ref="spec.store.SQLiteStore", spec_hash="a" * 64, file="file.py", line=10
    )
    claim2 = Implemented(
        ref="spec.store.SQLiteStore", spec_hash="a" * 64, file="file.py", line=20
    )
...
def test_jsonlines_store_idempotency_updates_current(tmp_path):
    ...
    comp_a = Component(
        ref="A", docstring="A", is_template=False, inherits=[], hash="a" * 64
    )
    comp_b = Component(
        ref="B", docstring="B", is_template=False, inherits=[], hash="b" * 64
    )
    ...
    snaps = [
        ...
        if json.loads(line_str)["type"] == "snapshot"
    ]
    comps = [
        json.loads(line_str)
        for line_str in lines
        if json.loads(line_str)["type"] == "component"
    ]
...
def test_jsonlines_store_delete_snapshot(tmp_path):
    ...
    comp_b = Component(
        ref="B", docstring="Doc B", is_template=False, inherits=[], hash="b" * 64
    )
...
def test_jsonlines_store_vcs_linking(tmp_path):
    ...
    store.store_vcs_link(
        snap.id, vcs="git", revision="new_resolved_commit", metadata={"branch": "main"}
    )
...
def test_jsonlines_store_component_dependencies(tmp_path):
    ...
    assert not any(
        e.get("type") == "dependency" and e.get("snapshot_id") == "PENDING"
        for e in events
    )
...
def test_jsonlines_store_is_dependency(tmp_path):
    ...
    legacy_comps = store_legacy.get_components_for_snapshot(
        store_legacy.list_snapshots()[0]
    )
