...
def test_xml_generation_idempotent(tmp_path):
    ...
    assert "date-created=" not in xml_content, (
        "generate_xml should not emit a timestamp for idempotency"
    )
    ...
    assert "date-created=" in written_content, (
        "write_xml should inject the timestamp temporarily before saving"
    )
    ...
    assert path1 == path2, (
        "identical specifications should generate the same filename hash"
    )
...
def test_consecutive_spec_generation_identical_filenames(tmp_path):
    ...
    assert os.path.basename(path1) == os.path.basename(path2), (
        "Two specs generated consecutively with identical content must map to perfectly identical filenames."
    )
