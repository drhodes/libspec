...
def pylsp_document_did_open(config, workspace, document):
    ...
    assert getattr(document, "source", None) is not None, (
        "document must have readable source code"
    )
...
def _scan_document(document, pattern, workspace):
    ...
    if matches:
        ...
        log.info(
            f"HelloAST: No identifiers matched pattern '{pattern}' in {document.path}"
        )
